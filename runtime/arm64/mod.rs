...
///
/// For ARM64, we let the OS choose the location.
...
pub fn translate_and_branch_to(
    ...
    // Always translate with returnable=false since we use sentinel-based return detection
    let block = translate_block(ctx, address, ctx.text_end, false).unwrap();
    if ctx.print_code {
        block.print_code();
    }
    if returnable {
