...
impl Assembler {
    ...
    pub fn emit_mov_indirect_to_reg64(&mut self, dst_reg: u8, insn: &iced_x86::Instruction) {
            ...
            && insn.memory_index() == iced_x86::Register::None
        {
            ...
            && insn.memory_index() != iced_x86::Register::None
        {
            // Base + index*scale + displacement addressing: MOV dst_reg, [base + index*scale + disp]
            let base_reg = insn.memory_base().number() as u8;
            let index_reg = insn.memory_index().number() as u8;
            ...
            // REX prefix: W=1, R=dst>=8, X=index>=8, B=base>=8
            let mut rex = 0x48; // REX.W
            if dst_reg >= 8 {
                rex |= 0x04; // REX.R
            }
            ...
            let disp = insn.memory_displacement64() as i64;
            if disp == 0 && (base_reg & 7) != 5 {
                // [base + index*scale] no displacement (RBP/R13 require disp8)
                let modrm = 0x00 | ((dst_reg & 7) << 3) | 0x04; // Mod=00, R/M=100 (SIB)
                let sib = (scale_bits << 6) | ((index_reg & 7) << 3) | (base_reg & 7);
                self.emit_u8(modrm);
                self.emit_u8(sib);
            } else if disp >= -128 && disp <= 127 {
                // [base + index*scale + disp8]
                let modrm = 0x40 | ((dst_reg & 7) << 3) | 0x04; // Mod=01, R/M=100 (SIB)
                let sib = (scale_bits << 6) | ((index_reg & 7) << 3) | (base_reg & 7);
                self.emit_u8(modrm);
                self.emit_u8(sib);
                self.emit_u8(disp as i8 as u8);
                ...
                let modrm = 0x80 | ((dst_reg & 7) << 3) | 0x04; // Mod=10, R/M=100 (SIB)
                let sib = (scale_bits << 6) | ((index_reg & 7) << 3) | (base_reg & 7);
                self.emit_u8(modrm);
                self.emit_u8(sib);
                self.emit_u32(disp as i32 as u32);
