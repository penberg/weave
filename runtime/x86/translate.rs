...
fn translate_insn(
    ...
    match insn.code() {
        ...
        _ => {
            ...
            // Guest code uses fs:[0] to get the TLS base pointer.
            // We intercept this and load from our guest TLS base instead.
            if insn.segment_prefix() == iced_x86::Register::FS {
                ...
                {
                    // mov reg, fs:[0] -> mov reg, [GUEST_FS_BASE_ptr]; mov reg, [reg]
                    // Load the FS base value (which is the self-pointer = TCB address)
                    let dest_reg = insn.op0_register();
                    trace!(
                        "TLS access: replacing mov {:?}, fs:[0] with guest FS base load",
                        dest_reg
                    );
                    // mov dest_reg, imm64 (address of GUEST_FS_BASE)
                    block.emit_mov_reg64_imm64(dest_reg, fs_base_addr);
                    // mov dest_reg, [dest_reg] (load the FS base value)
                    block.emit_mov_reg64_from_mem(dest_reg, dest_reg);
                } else {
                    return Err(Error::InstructionDecode(format!(
